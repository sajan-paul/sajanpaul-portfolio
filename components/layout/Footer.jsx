    ...
    <footer className="text-gray-900 dark:text-white relative bg-gradient-to-b from-transparent to-gray-50/50 dark:to-gray-900/30">
      ...
      <div className="container-custom">
        ...
        <div className="py-10 md:py-12">
          {/* Footer Layout - Unified for Mobile and Desktop */}
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 md:gap-8 lg:gap-12">
            {/* Brand Section with Logo and Media - Column 1 */}
                  ...
                  {orderedSocialIcons.map((social, index) => {
                    ...
                    return (
                      <motion.a
                        ...
                        className="w-12 h-12 rounded-full bg-white/10 dark:bg-gray-900/50 backdrop-blur-sm border border-white/20 dark:border-gray-700 flex items-center justify-center text-gray-700 dark:text-white hover:text-pink-400 hover:border-pink-400 transition-all duration-200 shadow-lg hover:shadow-xl hover:shadow-pink-500/20"
            ...
            {/* Quick Link and Contact - 2 columns on mobile, separate columns on desktop */}
                ...
                <ul className="space-y-3">
                  {quickAccessLinks.map((link, index) => (
                    ...
                    >
                      <a
                        href={link.href}
                        className="text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white transition-colors duration-200 text-sm flex items-center gap-2 group"
                      >
                        <span className="w-1.5 h-1.5 rounded-full bg-gray-400 dark:bg-gray-600 group-hover:bg-pink-400 dark:group-hover:bg-pink-400 transition-colors"></span>
                        {link.name}
                      </a>
                    </motion.li>
                  ))}
                ...
                <ul className="space-y-3">
                  ...
                  >
                    <a
                      ...
                      className="text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white transition-colors duration-200 text-sm flex items-center gap-2 group"
                    >
                      <Mail className="w-4 h-4 text-gray-500 dark:text-gray-400 group-hover:text-pink-400 transition-colors flex-shrink-0" />
                      <span className="flex-1 break-all">{bioData.email}</span>
                    </a>
                  </motion.li>
                  <motion.li
                    initial={{ opacity: 0, x: -10 }}
                  ...
                  >
                    ...
                    >
                      ...
                      <span className="flex-1">{bioData.phone}</span>
                    </a>
                  </motion.li>
                  <motion.li
                    initial={{ opacity: 0, x: -10 }}
                  ...
                  >
                    <div className="text-gray-600 dark:text-gray-400 text-sm flex items-center gap-2">
                      ...
                      <span className="flex-1">{bioData.location}</span>
                    </div>
                  </motion.li>
                </ul>
              </div>
